...
library ChmAffiliateCodes {
    ...
    function getCodeFromSeed(uint24 seed) internal pure returns (string memory) {
    ...
    function getSeedFromCode(string calldata code) internal pure returns (uint24) {
    ...
    function findCharacterIndex(bytes1 character) internal pure returns (uint8) {
        for (uint8 i = 0; i < ALLOWED_CHARACTERS_LENGTH; i++) {
            if (character == bytes(ALLOWED_CHARACTERS)[i]) {
                return i;
            }
        }
        return ALLOWED_CHARACTERS_LENGTH;
    }
    ...
    function generateSeed(address marketer, mapping(uint24 => address) storage existingSeeds)
        ...
        returns (uint24)
    {
        uint24 seed = uint24(uint160(marketer)) % MAX_SEED_COUNT;
